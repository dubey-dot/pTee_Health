...
└── backend/
    ...
    │           ├── rules.py              # load_assessment_rules() — reads assessment_rules.md fresh on every call
...
- **Where it lives**: `backend/app/services/engines/` — `base.py` (the `WorkingDiagnosisEngine` Protocol + `GeneratedAssessment`/`GeneratedInsightTag` schemas), `anthropic_client.py` (shared client, explicit missing-key guard), `working_diagnosis_engine.py` (`ClaudeWorkingDiagnosisEngine`, the real implementation, system prompt + structured-output call), `rules.py` + `assessment_rules.md` (centralized rules seam, see below).
- **Centralized rules system**: every call is governed by standing rules loaded from `assessment_rules.md`, a plain-Markdown file (currently a placeholder template — 6 sections, no real content yet) sent to Claude as its own system-prompt block (`RULES_PREAMBLE` instructs Claude to follow it before anything else, cached separately from the task-instruction block so either can change independently). `rules.py::load_assessment_rules()` reads the file fresh on every request — deliberately not cached — so editing the `.md` takes effect on the very next "Generate with AI" call with no code change and no backend restart. A missing file raises `RuntimeError`, already caught by the existing `502` handling in `api/v1/diagnosis.py` (no endpoint code needed changing).
