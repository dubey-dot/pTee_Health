...
│   │   │   │   ├── recommended-tests-panel.tsx  # rendered inside PteeAssistantPanel — batch feed, Accept/Reject/Undo, Selected Tests, useMutation against /assessments/{id}/recommendations
│   │   │   │   ├── log-test-panel.tsx
...
| `/api/v1/assessments/{assessment_id}/recommendations` | POST | Implemented — real Claude Opus 5 call recommends a ranked batch of up to 4 tests, each with `testName`/`summary`/`whyRecommended`/`confidence` (independent per-test confidence, not the overall diagnosis confidence — see Change Log). Advisory only, writes nothing to the DB. `502` on any Anthropic failure, same convention as `/diagnosis/generate` |
...
- **Recommendation is advisory only, not persisted**: `POST /assessments/{id}/recommendations` reads current findings/logged tests/doctor's notes fresh each call and returns a ranked batch of recommended tests — it never writes to the database or bumps the assessment's version. Accept/Reject decisions on the frontend are client-side-only state too (see Change Log) — there's no `RecommendationLog` table (the old PDF-derived plan flagged that entity as tentative). The doctor logs the actually-performed test through the existing findings/tests endpoints, and the next batch naturally sees it and won't repeat it.
...
- Recommendation Engine ("what to test next") — **now built**, real Claude call, one test at a time, see "AI & RAG Integration Progress" above; still reasons over today's flat findings/tests/notes data rather than the richer structured-findings model the PDF-derived plan describes (Foundational Assessments' full 3-plane taxonomy isn't captured as structured data yet, only referenced in the rules text — see `assessment_rules.md` § Foundational Assessments' "current implementation gap" note).
- No inline "log this test" shortcut from Recommended Tests, and no "add to plan" action for the Selected Tests panel — Accept currently only stages a test in local browser state (see Change Log); the doctor still logs the actually-performed test via the existing separate "Log a test" panel, typing the name themselves. Both a pre-filled hand-off and a real "add to plan" step were considered and deliberately deferred to keep this feature tightly scoped to what was asked each time.
